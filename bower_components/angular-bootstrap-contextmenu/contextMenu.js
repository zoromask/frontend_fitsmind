...
.directive('contextMenu', ["$parse", "$q", "CustomService", "$sce", function ($parse, $q, custom, $sce) {
    ...
    var processItem = function ($scope, event, model, item, $ul, $li, $promises, $q, $, level, container) {
        ...
        registerEnabledEvents($scope, isEnabled(), item, $ul, $li, nestedMenu, model, text, event, $, level, container);
    };

    var handlePromises = function ($ul, level, event, $promises, $contextMenu) {
        /// <summary>
        ...
        $q.all($promises).then(function () {
            if (level === 0) {
                var topCoordinate = event.pageY - $contextMenu.offset().top;
                var menuHeight = angular.element($ul[0]).prop('offsetHeight');
                var winHeight = event.view.innerHeight;
                if (topCoordinate > menuHeight && winHeight - topCoordinate < menuHeight) {
                    topCoordinate = event.pageY - $contextMenu.offset().top - menuHeight;
                }

                var leftCoordinate = event.pageX - $contextMenu.offset().left;
                var menuWidth = angular.element($ul[0]).prop('offsetWidth');
                var winWidth = event.view.innerWidth;
                if (leftCoordinate > menuWidth && winWidth - leftCoordinate < menuWidth) {
                    leftCoordinate = event.pageX - $contextMenu.offset().left - menuWidth;
                }
    ...
    var registerEnabledEvents = function ($scope, enabled, item, $ul, $li, nestedMenu, model, text, event, $, level, container) {
        ...
        if (enabled) {
            var openNestedMenu = function ($event) {
                ...
                renderContextMenu($scope, ev, nestedMenu, model, level + 1, container);
            };
    ...
    var renderContextMenu = function ($scope, event, options, model, level, container) {
        /// <summary>Render context menu recursively.</summary>
        if (!level) { level = 0; }
        if (!$) { var $ = angular.element; }
        $(event.currentTarget).addClass('context');
        var $contextMenu = $(event.currentTarget).parents("."+container);
        ...
        $ul.css({
            display: 'block',
            position: 'absolute',
            left: (event.pageX - $contextMenu.offset().left) + 'px',
            top: (event.pageY - $contextMenu.offset().top) + 'px',
            "z-index": 10000
        });
        ...
        angular.forEach(options, function (item) {
            ...
            if (item === null) {
                ...
                processItem($scope, event, model, item, $ul, $li, $promises, $q, $, level, container);
            }
            $ul.append($li);
        });
        ...
        handlePromises($ul, level, event, $promises, $contextMenu);

        $contextMenu.on("mousedown", function (e) {
            if ($(e.target).hasClass('grid-calendar-cell')) {
                $(event.currentTarget).removeClass('context');
                removeContextMenus();
            }
    ...
    return function ($scope, element, attrs) {
        element.on('click', function (event) {
            ...
            $scope.$apply(function () {
                ...
                var container = attrs.parentContainerSelector;
                if (options instanceof Array) {
                    if (options.length === 0) { return; }
                    renderContextMenu($scope, event, options, model, undefined, container);
                } else {
